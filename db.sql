...
(
    ...
    UNIQUE (username),
    CHECK (email LIKE '%@%.%'),
    CHECK (LENGTH(password) <= 30 AND LENGTH(password) >= 8),
...
CREATE TABLE orarioapertura(
    ...
    FOREIGN KEY (mensa) REFERENCES mensa(nome) ON UPDATE CASCADE ON DELETE CASCADE,
    CHECK (giornoSettimana >= 1 AND giornoSettimana <= 7),
...
FROM 
    mensa m
JOIN 
    orarioapertura oa ON m.nome = oa.mensa;

-- Inserimento dati di esempio

-- INSERT into utente (email, password, dataNascita, username) VALUES
-- ("user@example.com", "password", "1990-01-01", "user"),
-- ("user1@example.com", "password", "1990-01-01", "user1"),
-- ("user2@example.com", "password", "1990-01-01", "user2"),
-- ("admin@example.com", "password", "1995-02-15", "admin");

-- INSERT INTO mensa (nome, indirizzo) VALUES
-- ("Mensa Universitaria", "Via degli Studenti, 1"),
-- ("Mensa del Lavoratore", "Corso del Lavoro, 20"),
-- ("Mensa Vegana", "Via Verde, 15");

-- INSERT INTO piatto (nome, descrizione) VALUES
-- ("Pasta al Pomodoro", "Spaghetti con salsa di pomodoro fresco."),
-- ("Insalata Mista", "Insalata con verdure fresche e vinaigrette."),
-- ("Riso Vegano", "Riso con verdure e spezie, completamente vegano."),
-- ("Pollo alla Griglia", "Petto di pollo grigliato servito con contorno."),
-- ("Zuppa di Lenticchie", "Zuppa calda di lenticchie, perfetta per l'inverno.");

-- INSERT INTO menu (data, mensa) VALUES
-- ("2024-10-01", "Mensa Universitaria"),
-- ("2024-10-01", "Mensa del Lavoratore"),
-- ("2024-10-01", "Mensa Vegana");

-- INSERT INTO menu_piatto (piatto, data, mensa) VALUES
-- ("Pasta al Pomodoro", "2024-10-01", "Mensa Universitaria"),
-- ("Insalata Mista", "2024-10-01", "Mensa Universitaria"),
-- ("Pollo alla Griglia", "2024-10-01", "Mensa del Lavoratore"),
-- ("Zuppa di Lenticchie", "2024-10-01", "Mensa del Lavoratore"),
-- ("Riso Vegano", "2024-10-01", "Mensa Vegana");

-- INSERT INTO recensione (voto, descrizione, utente, piatto) VALUES
-- (4, "Buona qualità dei piatti, ma a volte c'è attesa.", "user@example.com", "Pasta al Pomodoro"),
-- (5, "Ottima mensa! Servizio veloce e cibo delizioso.", "user1@example.com", "Pollo alla Griglia"),
-- (3, "Buoni piatti, ma un po' costosi.", "user2@example.com", "Riso Vegano"),
-- (5, "Ottima pasta! Salsa fresca e saporita.", "user2@example.com", "Pasta al Pomodoro"),
-- (4, "Insalata buona, ma potrebbe avere più varietà.", "user1@example.com", "Insalata Mista"),
-- (5, "Riso delizioso! Consigliato!", "user1@example.com", "Riso Vegano"),
-- (4, "Pollo alla griglia ben cotto e saporito.", "user2@example.com", "Pollo alla Griglia");

-- INSERT into orarioapertura (giornoSettimana, orainizio, orafine, mensa) VALUES
